...
#include "../Includes/Socket.class.hpp"
...
Socket::Socket(const std::string &ipAddr, const int &port)
    : _ipAddress(ipAddr), _port(port), _sock(), _socket(0),
      _vectorServerNameToken(), _server(NULL) {}

// Socket::Socket(epoll_event & event, Socket * server)
//: _ipAddress(), _port(), _sock(), _socket(event.data.fd),
//: _vectorServerNameToken(server->_vectorServerNameToken),
//_server(server){
//    buildClientSocket();
//}
...
Socket::Socket(const Socket &other)
    : _ipAddress(other._ipAddress), _port(other._port), _sock(other._sock),
      _socket(other._socket),
      _vectorServerNameToken(other._vectorServerNameToken),
...
bool Socket::checkSocket(int fd) {
  ...
  getsockname(fd, reinterpret_cast<struct sockaddr *>(&address),
              &address_length);
  string ipAddress = inet_ntoa(_sock.sin_addr);
  int port = ntohs(_sock.sin_port);
  if (port == _port && ipAddress == _ipAddress)
    return true;
  return ((_sock.sin_addr.s_addr == address.sin_addr.s_addr) &&
          (_sock.sin_port == address.sin_port));
}
...
void Socket::setSocketOption() {
  int optionVal = 1;
  if (setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR, &optionVal,
                 (socklen_t)sizeof(optionVal))) {
    close(_socket);
    throw Exception(strerror(errno), 503);
  }
}
...
void Socket::setBind() {
  ...
  if (bind(_socket, reinterpret_cast<struct sockaddr *>(&_sock), addressLen) <
      0) {
    close(_socket);
    throw Exception(strerror(errno), 503);
  }
...
void Socket::acceptConnection() {
  socklen_t addressLen = sizeof(_sock);
  int fd =
      accept(_socket, reinterpret_cast<struct sockaddr *>(&_sock), &addressLen);
  if (fd == -1) {
    closeSocket();
    throw Exception(strerror(errno), 503);
  }
...
int Socket::getSocket() const { return _socket; }
...
void Socket::closeSocket() const { close(_socket); }
...
const std::string &Socket::getIpAddress() const { return _ipAddress; }

void Socket::addToken(const string &defaultName, const string &token) {
  _vectorServerNameToken.push_back(make_pair(defaultName, token));
}

void Socket::addServerName(std::vector<std::string> &name,
                           const std::string &token) {
  for (vecPairStrStrIt it = _vectorServerNameToken.begin();
...
vecPairStrStr &Socket::getVectorServerNameToken() {
  return _vectorServerNameToken;
}

Socket *Socket::getServer() const { return _server; }
