...
bool splitBlocks(vecStr &split, const string &str, const string &pattern,
    ...
    while (getline(isStr, line) && ret) {
        if (line.find(pattern) != string::npos) {
            ...
            if (tmpSplit[0] == pattern &&
                tmpSplit[pattern == "server" ? 1 : 2] == "{" &&
...
static string removeCommentary(string &line) {
    size_t commentPos;
    ...
    if (commentPos == 0)
        ...
        cout << line << "size" << commentPos << line.size() <<  vendl;
...
//  if (!isPropertyNameValid(result[0],result object == "server" ? server_properties
//                                                         : route_properties))
    throw ParsingException(line, "'" + result[0] +
                                 "' is not a valid property for " + object +
                                 ".");
    return (result);
}
